...
class RLHFDataset(Dataset):
    ...
    def _process_values(self):
        ...
        if self.eta < 0 or 'eval_0' not in columns:
            ...
            print(f"v_star is set to mean: num_gen_to_use - {self.num_gen_to_use}")
        else:
            selected_cols = [f"eval_{i}" for i in range(self.num_gen_to_use)]
            v_star = self.dataframe[selected_cols].to_numpy()
            if self.normalize_rw:
                v_star = v_star - 0.5
            v_star = np.exp(self.eta * v_star).mean(axis=1)
            v_star = np.log(v_star) / self.eta
            print(f"v_star is set to exp: num_gen_to_use - {self.num_gen_to_use}, eta - {self.eta}")
    ...
    def _filter_dataset(self):
        ...
        if self.filter_correct:
            assert self.num_gen_to_use > 0
            selected_cols = [f"eval_{i}" for i in range(self.num_gen_to_use)]
            evals = self.dataframe[selected_cols].to_numpy()
            evals = np.mean(evals, axis=1)
            self.dataframe = self.dataframe[evals != 1]
            print(f'after filtering prompts with all correct responses: {len(self.dataframe)}')
