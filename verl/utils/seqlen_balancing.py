...
def rebel_rearrange_micro_batches(batch: TensorDict):
    """ REarrange such that the set of 2 has the same input_ids """
    """ REarrange such that the set of 2 has the same input_ids """
    # Get the input_ids from the batch
    input_ids = batch['input_ids']
    batch_size = input_ids.shape[0]
    
    # Create a list to store pairs of indices with same input_ids
    pairs = []
    used = set()
    
    # Find pairs with same input_ids
    for i in range(batch_size):
        ...
        for j in range(i + 1, batch_size):
            if j in used:
                continue
            if torch.equal(input_ids[i][:256], input_ids[j][:256]):
                pairs.append((i, j))
                used.add(i)
                used.add(j)
                break
    
    # Create new batch with rearranged indices
    new_batch = copy.deepcopy(batch)
    new_indices = []
    
    # Add pairs first
    for i, j in pairs:
        new_indices.extend([i, j])
    ...
    # Rearrange batch according to new indices
    for key in new_batch.keys():
        if isinstance(new_batch[key], torch.Tensor):
            new_batch[key] = new_batch[key][new_indices]
    
    print("rearranged batch")
    return new_batch, new_indices
