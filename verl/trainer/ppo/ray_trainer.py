...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == 'gae':
    ...
    elif adv_estimator == 'grpo' or adv_estimator == 'sft':
        token_level_rewards = data.batch['token_level_rewards']
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == 'rloo' or adv_estimator == 'sft':
        token_level_rewards = data.batch['token_level_rewards']
        index = data.non_tensor_batch['uid']
        responses = data.batch['responses']
        response_length = responses.size(-1)
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_rebel_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
...
def compute_data_metrics(batch, use_critic=True, true_correctness_list=None):
    ...
    metrics = {
        'critic/true_correctness/mean':
            torch.mean(true_correctness_list).detach().item(),
        'critic/true_correctness/max':
            torch.max(true_correctness_list).detach().item(),
        'critic/true_correctness/min':
            torch.min(true_correctness_list).detach().item(),
        # score
        'critic/score/mean':
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            format_score_tensor, correctness_score_tensor, recorded_sequence_reward, true_correctness_list = self.val_reward_fn(test_batch)
    ...
    def init_workers(self):
        ...
        elif self.config.algorithm.adv_estimator == 'grpo' or self.config.algorithm.adv_estimator == 'rloo' or self.config.algorithm.adv_estimator == 'rebel' or self.config.algorithm.adv_estimator == 'sft':
            assert self.config.data.filter_time == 1
        ...
        for epoch in range(int(self.config.trainer.total_epochs * self.config.data.filter_time)):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # if not rebel, we balance the batch
                    if self.config.algorithm.adv_estimator != 'rebel':
                        self._balance_batch(batch, metrics=metrics)

                    # compute global_valid tokens
                    batch.meta_info['global_token_num'] = torch.sum(batch.batch['attention_mask'], dim=-1).tolist()
                    batch.meta_info['rew_type'] = self.config.reward_model.rew_type
                    if self.use_reference_policy:
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        format_score_tensor, correctness_score_tensor, _, true_correctness_list = self.reward_fn(batch)
                        true_correctness_list = torch.tensor(true_correctness_list)
                ...
                # collect metrics
                metrics.update(compute_data_metrics(batch=batch, use_critic=self.use_critic, true_correctness_list=true_correctness_list))
